...
export function useMqtt() {
  ...
  useEffect(() => {
    ...
    let socketUrl: string | undefined = import.meta.env.VITE_SOCKET_URL;
    
    // Jika tidak ada URL di env, gunakan Railway HANYA jika berjalan di Vercel
    if (!socketUrl) {
      if (typeof window !== 'undefined' && window.location.hostname.includes('vercel.app')) {
        socketUrl = "https://multibroker-fariq-efriadi-production.up.railway.app";
      } else {
        // Untuk AI Studio atau localhost, ke backend (Express) yang sama
        socketUrl = undefined;
      }
    }
